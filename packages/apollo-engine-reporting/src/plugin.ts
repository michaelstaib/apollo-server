...
import { EngineReportingOptions, AddTraceArgs, GenerateClientInfo, SendValuesBaseOptions, VariableValueOptions } from "./agent";
import { Logger, GraphQLRequestContext } from "apollo-server-types";
...
export const plugin = <TContext>(
  ...
  return {
    requestDidStart(requestContext) {
      let explicitOperationName: string | null | undefined;
      ...
      return () => {
        ...
        treeBuilder.trace.fullQueryCacheHit = !!metrics
          .responseCacheHit;
        treeBuilder.trace.forbiddenOperation = !!metrics
          .forbiddenOperation;
        treeBuilder.trace.registeredOperation = !!metrics
          .registeredOperation;
        ...
        const operationName =
          explicitOperationName || requestContext.operationName || '';
